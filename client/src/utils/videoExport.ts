...
function drawTextLines(
  ...
  // Extract font size from ctx.font
  ...
  // Draw background box for badge preset
  if (preset === "badge") {
    ctx.save();
...
export async function exportSlideshowToVideo(slideshow: Slideshow): Promise<void> {
  const width = 576;
  const height = 1024;
  ...
  const recordingPromise = new Promise<void>((resolve, reject) => {
    mediaRecorder.onstop = () => {
      ...
      a.href = url;
      a.download = `slideshow-${slideshow.id}.${ext}`;
      a.click();
  ...
  for (let i = 0; i < slideshow.slides.length; i++) {
    ...
    const framesForSlide = Math.floor((SLIDE_DURATION_MS / 1000) * FPS);
    for (let frame = 0; frame < framesForSlide; frame++) {
      ...
      for (const overlay of slide.textOverlays) {
        ...
        if (overlay.stylePreset === "neon") {
          ...
          ctx.shadowBlur = 0;
          ctx.shadowOffsetY = 0;
          ctx.fillStyle = "white"; // White text in badge
